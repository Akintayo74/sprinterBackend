...
export class CardController {
  ...
  @Patch('cards/:cardId/comments/:id')
  async update(
    @Req() req: Request & { user?: { id: number } },
    @Param('id') id: number,
    @Body() dto: CreateCommentDto,
  ) {
    return this.cardService.updateComment(req.user!.id, +id, dto);
  }

  @Delete('cards/:cardId/comments/:id')
  remove(
    @Req() req: Request & { user?: { id: number } },
    @Param('id') id: number,
  ) {
  return this.cardService.remove(req.user!.id, +id);
  }
}
